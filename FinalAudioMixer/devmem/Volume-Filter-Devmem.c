...
#define AUDIO_OUT_REG *((unsigned *)(audioOutPtr + 0))
...
//...............For UDP Receiver...............
  ...
  //-------------------------------------------------------------------------------
	...
	unsigned addressV= 0x43C20000;
	if(channel == 0)
		addressV = address00; 
	else
		addressV = address11; 
	//addressV = channel == 0 ? address00 : address11;       
...
//-----------------------------------------------------------------------
	...
	unsigned addressF= 0x43C10000;
	if(channel == 0)
		addressF = address0; 
	else
		addressF = address1; 
        // choose channel for filter
        //unsigned addressF = channel == 0 ? address0 : address1;
	...
	//mapp channel address for Filter
        ...
        unsigned offsetF = (addressF & (~(pageSizeF - 1)));

        printf("Mapping address for Filter\n");
        void *ptr = mmap(NULL, pageSizeF, PROT_READ | PROT_WRITE, MAP_SHARED, fd, offsetF);
	...
	while(channel == 0 || channel == 1)
	{
		...
		if ( channel ==1)
		{
			...
			{ 	
				...
				SLV_REG_18 = 1;
				SLV_REG_17 = 0;										
				oled_print_message( "Channel1LOW", 0, ptr1) ;
			...
			else
			{
				...
				oled_print_message( "Channel1HIGH", 0, ptr1) ;
			}
		}	
